...
data "aws_security_group" "DevOps-SecGrp001" {
  tags = {
    Name = "DevOps-SecGrp001"
  }
}
...
resource "aws_instance" "server1" {
  ...
  vpc_security_group_ids      = [data.aws_security_group.DevOps-SecGrp001.id]

  tags = {
    Name = "server1"
  }

  key_name                    = "DevOps_KEYS_001"
  user_data                   = <<-EOF
                #! /bin/bash
                yum update -y
                yum install httpd -y
